...
class Servidor:
    ...
    def aceptar_clientes(self):
        ...
        while True:
            ...
            diccionario_jugador = self.decodificar_mensaje(data)
            print(diccionario_jugador)
            # jugador = Jugador(diccionario_jugador["username"])
            jugador = Jugador(diccionario_jugador)
            jugador.socket_cliente = socket_cliente
            jugador.address = address
            self.lista_jugadores.append(jugador)
            escuchando_clientes = threading.Thread(
                target=self.escuchar_cliente,
                args=(jugador, ),
                daemon=True)
            escuchando_clientes.start()
    ...
    def escuchar_cliente(self, jugador):
        ...
        """
        try:
            socket_cliente = jugador.socket_cliente
            data = socket_cliente.recv(4096)
            mensaje = self.decodificar_mensaje(data)
    ...
    def enviar(self, mensaje, socket_cliente):
        ...
        """
        mensaje_bytes = mensaje.codificar_mensaje()
        socket_cliente.send(mensaje_bytes)
    ...
    def recibir(self, socket_cliente):
        ...
        # Completar
        while True:
            largo_bytes_mensaje = self.socket_cliente.recv(5)
            largo_mensaje = int.from_bytes(
                largo_bytes_mensaje, byteorder='little')
            mensaje = bytearray()

            while len(mensaje) < largo_mensaje:
                largo_lectura = min(64, largo_mensaje - len(mensaje))
                mensaje.extend(self.socket_cliente.recv(largo_lectura))
