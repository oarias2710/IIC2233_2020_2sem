...
class RedesMundiales:
    ...
    def agregar_aeropuerto(self, aeropuerto_id, nombre):  # LISTO
    ...
    def agregar_conexion(self, aeropuerto_id_partida, aeropuerto_id_llegada, infectados):  # LISTO
        # Crear la conexion de partida-llegada para el par de aeropuertos
        if aeropuerto_id_partida in self.aeropuertos.keys():
            nueva_conexion = Conexion(aeropuerto_id_partida, aeropuerto_id_llegada, infectados)
            self.aeropuertos[aeropuerto_id_partida].conexiones.append(nueva_conexion)
    ...
    def infectados_generados_desde_aeropuerto(self, aeropuerto_id):  # HACER
        # Muestra la cantidad de infectados generados por un aeropuerto
        aeropuertos_visitados = [] # set()  # registro de los id de aeropuertos visitados
        infectados = 0  # contador de infectados
        stack = [aeropuerto_id]  # partimos por este id
        while len(stack) > 0:
            id_aeropuerto_origen = stack.pop()
            # Si ya contamos el aeropuerto, no se hace nada
            if id_aeropuerto_origen in aeropuertos_visitados:
                continue
            # Si no lo hemos contado, lo agregamos a la lista
            aeropuertos_visitados.append(id_aeropuerto_origen)
            # Y sumamos al contador los infectados de sus conexiones
            for elem in self.aeropuertos[id_aeropuerto_origen].conexiones:
                infectados = infectados + elem.infectados
                if elem.aeropuerto_llegada_id not in aeropuertos_visitados:
                    stack.append(elem.aeropuerto_llegada_id)
        # print(aeropuertos_visitados)
        print(f"La cantidad est. de infectados gen. por el aerop {self.aeropuertos[aeropuerto_id].nombre} es de {infectados}")
        return infectados

    def verificar_candidatos(self, ruta_aeropuertos_candidatos, ruta_conexiones_candidatas):  # HACER
...
if __name__ == "__main__":
    ...
    # Para el aeropuerto 4 debería ser 30000
